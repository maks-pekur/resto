...
5. A public HTTPS endpoint exists for Stripe webhooks before Phase 8 begins; an external smoke confirms all four surfaces are reachable over HTTPS (api `/healthz`, admin login page, website tenant menu, qr-menu SPA) and a tenant menu renders end-to-end
   **Plans**: 11 plans — `01`–`05` and `11` executed (see their SUMMARYs), `06`–`10` re-planned 2026-09-04 for the VPS + Cloudflare stack and remediated the same day against `07.5-VERIFICATION.md`
...
- [x] 07.5-07-PLAN.md — the two Cloudflare Workers (admin + qr-menu): same-origin `/v1/*` (+`/api/*`) proxy, `X-Forwarded-Host` tenant preservation, and per-tenant edge cache keys with cross-tenant isolation proven by test; retires the admin Dockerfile

**Wave 2** _(the live box — depends on 06's compose files and 07's Workers)_

- [ ] 07.5-08-PLAN.md — VPS + Cloudflare stand-up: bootstrap + outside-in verifier scripts, founder provisioning (Hetzner, DNS, Origin CA, Full-strict, R2 ×2), then first live bring-up — roles, migrations, preflights under real credentials, both Workers on air (SC#1/#2/#4/#5)

**Wave 3** _(CD — depends on the live box)_

- [ ] 07.5-09-PLAN.md — `deploy.yml` on merge to main: GHCR build → fail-closed input assertion (`DATABASE_ADMIN_URL` only warns in `migrate.ts`) → gated migration over SSH → `up -d --no-deps` → `wrangler deploy` ×2 → `/healthz` poll; volume guard wired into CI; app-only rollback proven (SC#3; D-10; G-06)

**Wave 4** _(go-live gate)_

- [ ] 07.5-10-PLAN.md — nightly backups on cron + R2 lifecycle + a restore drill performed from a real R2 object (G-02); production data audit clearing the dev-tunnel `tenant_domains` row that would print itself onto every QR sticker; external four-surface smoke incl. live cross-tenant menu isolation and webhook reachability; go-live checklist; G-07 supersession of the AWS-era infra docs
      **UI hint**: no

...
- AUTH-09 (scoped): idempotent role-seeding (NestJS bootstrap step or generated SQL migration). NO role-change endpoint. BLOCKED row in audit-gap.md stays BLOCKED with explicit re-eval trigger.
...
- In-place role-change UI + custom NestJS endpoint via `auth.api.updateMemberRole` (Phase 17 / TEAM-03)

...
| 3   | AUTH-09 role-change endpoint — keep direct mutation, switch to BA server-side API, or drop entirely | **Drop entirely** (Skeptic variant) | Phase 3 ships role-seeding only. BLOCKED row in `audit-gap.md` stays BLOCKED with explicit re-eval trigger documented (first multi-member tenant OR BA ≥ 1.5 hook). Full role-change UI + endpoint move to Phase 17 / TEAM-03 via `auth.api.updateMemberRole(...)`. |

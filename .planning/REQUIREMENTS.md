...
- [~] **ADM-04**: ~~Operator can create a new brand inside their current tenant~~ — SUPERSEDED by Phase 10.2 (D-07: the brands table is dropped, one organization = one restaurant). No replacement entry point ships this phase; CONTEXT.md records the deferred create-organization GAP.
- [~] **ADM-05**: ~~Operator can switch active brand; active-brand state persists in signed cookie~~ — SUPERSEDED by Phase 10.2 (D-10: brand-switching code deleted; D-14/D-15: the active organization is the session's entire context, held by Better Auth, not a signed cookie — CONTEXT.md's "Rejected before" note explicitly rejects reintroducing a context cookie).
...
- [x] **ORDINT-10**: Public `GET /v1/orders/:id/status` endpoint (or SSE stream) returns current order state (`accepted / preparing / ready / on its way`); used by guest-facing confirmation page for live status polling
...
| ADM-04      | Phase 2       | Superseded (Phase 10.2) |
| ADM-05      | Phase 2       | Superseded (Phase 10.2) |

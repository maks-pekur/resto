...
| 3   | `LocationScopeGuard` | `APP_GUARD`                                                                                                                                                             | Default-on: 403 `location.context_required` if no location resolved; owner `baseRole` bypasses; otherwise checks session-pinned `activeLocationId` matches AND `member_location_scope` includes it | `@LocationNeutral()` opt-out                                                                                 |
| 4   | `OwnerOnlyGuard`     | `APP_GUARD`                                                                                                                                                             | No-op unless `@OwnerOnly()` present; then requires `principal.baseRole === 'owner'`, else 404 (not 403 — hides existence)                                                               | No decorator = pass-through                                                                                  |
| 5   | `RateLimitGuard`     | `app.useGlobalGuards()` in `apps/api/src/shared/security.ts:277`, called from `main.ts` _after_ `NestFactory.create()` has already resolved the `APP_GUARD` providers above | Invokes the Fastify `@fastify/rate-limit` handler from inside the Nest pipeline so 429s get `ProblemDetailsFilter` formatting                                                           | `allowList` in `security.ts` (health check, Stripe webhook)                                                  |

All five live in one `ApplicationConfig.globalGuards` array and run in registration order via `GuardsContextCreator`/`GuardsConsumer` (`@nestjs/core`), short-circuiting on the first guard that returns `false`/throws. Because `APP_GUARD` providers are resolved during `NestFactory.create()` (before `registerSecurity()` runs), **`AuthGuard` always executes before `RateLimitGuard`** — a rate-limited unauthenticated request still pays the BA session lookup cost. Registration source: `apps/api/src/contexts/identity/identity-http.module.ts:86-89` (providers array, guards 1→4 in that literal order) and `apps/api/src/shared/security.ts:277`.

**There is no brand guard.** Phase 10.2 removed the Brand level from the product, and with it `BrandScopeGuard`, `@BrandNeutral()` and `@RequireBrand()`. `LocationScopeGuard` reads `LOCATION_NEUTRAL_KEY` and nothing else (`location-scope.guard.ts:17,27`); the old dual-key behavior, where a brand-neutral route was also location-neutral, is gone with it. If you find `@BrandNeutral()` in an older plan, summary or review document, it describes a system that no longer exists — do not add it to a controller.
...
| `@AllowArchivedTenant()`   | `tenancy:allow_archived_tenant`    | `AuthGuard`                                                       | Lets the route through for a tenant that is archived, which `AuthGuard` otherwise 404s (`auth.guard.ts:70`)                                                        |
| `@Permissions(spec)`       | `identity:permissions`             | `PermissionsGuard`                                                | Requires the named permission via `BetterAuthPermissionChecker`                                                                                                    |
| `@LocationNeutral()`       | `identity:location-neutral`        | `LocationScopeGuard`                                              | Opts a route out of location scoping — the only opt-out left now that the brand guard is gone                                                                      |
...
**The incident this table exists to prevent:** `LocationScopeGuard` shipped default-on (phase 08.4-05) without auditing the controllers that were already opted out of the then-existing brand guard. Every one of them 403'd on `location.context_required` _before_ the owner-bypass check — including guest menu reads, guest checkout (`POST /v1/orders`), signup and tenant provisioning. The lesson survives the brand guard that caused it: **a guard that ships default-on must be audited against every route that expects to be exempt, before it ships — not after.** When adding a controller today, the only question is whether it needs `@LocationNeutral()`; a route reachable without a bound location does. Full incident record: `.planning/phases/08.4-location-scoped-access/deferred-items.md` and `08.4-06-SUMMARY.md`.

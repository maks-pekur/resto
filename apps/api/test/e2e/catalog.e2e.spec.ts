...
suite('Catalog — authed write → public read → cross-tenant isolation', () => {
  ...
  it('menu ETag advances after publish (version counter intact)', async () => {
    const categoryRes = await stack.app.inject({
      method: 'POST',
      url: '/v1/catalog/categories',
      headers: cafeA.authed,
      payload: { slug: 'etag-cat', name: { en: 'ETag Cat' }, sortOrder: 0 },
    });
    const categoryId = categoryRes.json<{ id: string }>().id;
    ...
    await stack.app.inject({ method: 'POST', url: '/v1/catalog/publish', headers: cafeA.authed });

...
import { requireBrandContext, requireLocationContext, requireTenantContext } from '@resto/db';
...
export class GetStopListService {
  ...
  async execute(): Promise<StopListResponse> {
    ...
    // a resolved x-location-id belongs to the active brand. Existence-hiding
    ...
    const active = (await this.locations.listForBrand(brandId, tenantId)).filter(
      (l) => l.status === 'active',
    );

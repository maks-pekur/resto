...
import { isSubsetOf } from '@resto/domain';
import { resolveEffectivePermissions } from './resolve-effective-permissions';
...
export class LocationPermissionChecker implements PermissionChecker {
  ...
  async hasPermission(
    ...
    const effective = await resolveEffectivePermissions(this.authDb, this.reader, {

-- Phase 08.4 Plan 08: tenancy_erase_tenant never touched locations,
-- member_location_scope, or catalog_location_stop_version. Plan 06
-- re-keyed catalog_location_stop_version from a brand-cascade FK to a
-- location-RESTRICT FK (0068_stop_list_location_regrain.sql), and this
-- plan adds orders_location_fk (also RESTRICT) -- so DELETE FROM brands
-- now fails once a tenant has any location, and DELETE FROM locations
-- (added below) would itself fail if a stop-list entry still references
-- it. Delete child-first: catalog_location_stop_version + (explicit,
-- mirroring member_brand_scope's own explicit line even though the
-- memberId FK already cascades) member_location_scope, then locations,
-- before the existing brands block.

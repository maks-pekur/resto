-- API review 2026-06-15 BLOCK-2: tenancy_erase_tenant never touched the ordering
-- tables (added in 0049). Guest PII (customer_name/phone, table_identifier) and
-- the Stripe provider_payment_id survived erasure, and orders.brand_id is
-- ON DELETE RESTRICT to brands, so erasing a tenant with orders failed at
-- DELETE FROM brands. Delete the ordering tables (child-first) before brands.

...
export const isDockerAvailable = (): boolean => {
  try {
    ...
    if (flag === '1' || flag === 'true') {
      throw new Error(
        'RESTO_REQUIRE_DOCKER is set but Docker is not available — refusing to silently skip the tenant-isolation test suite. Start Docker or unset RESTO_REQUIRE_DOCKER.',
      );
    }
    return false;
  }

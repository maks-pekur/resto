  ...
  return (
    <div className={cn('bg-muted relative flex rounded-full p-0.5', className)}>
      ...
      {options.map((option) => (
        <label
          key={option.id}
          className="has-[:checked]:text-foreground text-muted-foreground has-[:focus-visible]:ring-ring relative z-10 flex flex-1 cursor-pointer flex-col items-center rounded-full px-3 py-1.5 text-center text-xs font-bold transition-colors has-[:focus-visible]:ring-2"
        >
          <input
            type="radio"
            className="sr-only"
